...
class XiPTHelperSetup 
{
	...
	function syncUpUserPTRequired()
	{
		...
		$PTFieldId = XiPTLibraryProfiletypes::getFieldId(PROFILETYPE_CUSTOM_FIELD_CODE);
		$TMFieldId = XiPTLibraryProfiletypes::getFieldId(TEMPLATE_CUSTOM_FIELD_CODE);
		
		// we need first these fields to be exist
		if(!($PTFieldId && $TMFieldId))
			return true;
			
		$result = XiPTLibraryCore::getUsertoSyncUp();
	...
	function syncUpUserPT($start, $limit, $test = false)
	{
		...
		$result = XiPTLibraryCore::getUsertoSyncUp($start, $limit);
