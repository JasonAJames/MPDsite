...
class XiPTLibraryProfiletypes
{
	...
	function getProfiletypeArray($filter='')
	{
		...
		$results[$hash] = $db->loadObjectList();
	...
	{	
	    ...
	    $db		= JFactory::getDBO();
		$query	= 'SELECT * FROM '
				. $db->nameQuote( '#__xipt_users') 
				. " WHERE `userid` >= $userid " 
				. ' LIMIT 100';
		$db->setQuery( $query );
		
		//JDatabase
		$result	= $db->loadAssocList('userid');
		
		//echo "\n Records Loaded ".count($result);
		//print_r($result);
	...
	function getProfiletypeData($id=0, $what='name')
	{
		...
		$reset = XiPTLibraryCore::cleanStaticCache();
		...
		if( isset($results[$id]) && isset($results[$id][$searchFor]) && $reset === false)
			return $results[$id][$searchFor];
...
// 		return $notselected;
	...
	function getParams($id,$what='params')
	{
		...
		$pTypeConfig = null;
		if(isset($result[$id]) && isset($result[$id][$what]))
			$pTypeConfig = $result[$id][$what];
