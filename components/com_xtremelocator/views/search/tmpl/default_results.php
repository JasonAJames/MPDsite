...
<div id="<?php echo $this->css['xl_search_results']['class'];?>">
        ...
        foreach ($lok as $k => $v)           
            {
                ...
                if($k!=''&&$k!='Id'){                    
                            ...
                            if(!isset($_GET['lid'])&& $v['lincable']==1){
                                $row[$k]="<a href='".JRoute::_( 'index.php?option=com_xtremelocator&view=search')."&lid=".$i."'>".$row[$k]."</a>";
                            }               
    ...
    }elseif(!isset($_GET['lid'])){ echo '<div id="'.$this->css['xl_search_footer']['class'].'">'.($position>0?"<a href='index.php?option=com_xtremelocator&view=search&pos=".($position - $pageSize)."'>&lt; ".JText::_('PREVIOUS')."</a> ":"").JText::_('RECORDS').' ' .($start + 1)." - ".($end)." (".count($r).")".($position + $pageSize < count($r)?"<a href='index.php?option=com_xtremelocator&view=search&pos=".($position+$pageSize)."'> ".JText::_('NEXT')."&gt;</a>":"").'</div>';};?>
